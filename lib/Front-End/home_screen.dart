...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Values>>(
      future: valuesFuture,
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          final fetchedValues = snapshot.data ?? [];
          if (fetchedValues.isNotEmpty) {
            lastValue = fetchedValues.last;
          }
          return Stack(
            children: [
              const Padding(
                  ...
                  Align(
                    ...
                    child: GestureDetector(
                      ...
                      child: ProgressCircle(
                          (lastValue?.y ?? 0) / 100, 150), // Modified line
                    ),
                  ),
                  const SizedBox(
                    height: 20,
